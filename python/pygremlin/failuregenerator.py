...
import httplib  # FIXME: python 3 does not have httplib
import json
import logging
import uuid
...
from pygremlin.logger import logger
...
class FailureGenerator(object):
    def __init__(self, app, debug=False):
        ...
        :param app: instance of ApplicationGraph object
        """
        self.app = app
        self.debug = debug
        self._id = None
    ...
    def _notify_proxies(self):
        if self.debug:
            logger.debug('in _notify_proxies')
        ...
        for service in self.app.get_services():
            ...
            for instance in self.app.get_service_instances(service):
                resp = requests.get(
                    "http://{}/gremlin/v1/test/{}".format(instance, self._id))
    ...
    def add_rule(self, **args):
        ...
        assert myrule['delayprobability'] > 0.0 or \
               myrule['abortprobability'] > 0.0 or \
               myrule['mangleprobability'] > 0.0
        if myrule["delayprobability"] > 0.0:
            assert myrule["delaytime"] != ""
        if myrule["abortprobability"] > 0.0:
            assert myrule["errorcode"] >= -1
        assert myrule["messagetype"] in ["request", "response", "publish",
                                         "subscribe"]
    ...
    def clear_rules_from_all_proxies(self):
        ...
        for service in self.app.get_services():
            for instance in self.app.get_service_instances(service):
                if self.debug:
                    logger.debug('Clearing rules for %s - instance %s' % (
                        service, instance))
                resp = requests.delete(
                    "http://{}/gremlin/v1/rules".format(instance))
                if resp.status_code != 200:
                    logger.info('Failed to clear rules for %s - instance %s' % (
                        service, instance))

    def list_rules(self, service=None):
        """
            List fault fault injection rules installed on instances of a given
            service (or all services) returns a JSON dictionary
        """
        rules = {}
        for service in self.app.get_services():
            rules[service] = {}
            for instance in self.app.get_service_instances(service):
                ...
                resp = requests.get(
                    "http://{}/gremlin/v1/rules/list".format(instance))
                if resp.status_code != 200:
                    logger.warn('Failed to fetch rules from %s - instance %s'
                                % (service, instance))
                    continue
    ...
    def push_rules(self, continue_on_errors=False):
        for rule in self._queue:
            ...
            for instance in instances:
                try:
                    resp = requests.post(
                        "http://{}/gremlin/v1/rules/add".format(instance),
                        headers={"Content-Type": "application/json"},
                        data=json.dumps(rule))
                    resp.raise_for_status()
                except requests.exceptions.ConnectionError as e:
                    logger.error("FAILURE: Could not add rule to instance"
                                 " %s of service %s" % (instance,
                                                        rule["source"]))
                    logger.error(e)
    ...
    def _generate_rules(self, rtype, **args):
        rule = args.copy()
        assert rtype is not None and rtype != "" and (
            rtype is "delay" or rtype is "abort")
        ...
        for s in sources:
            for d in destinations:
                ...
                self.add_rule(**rule)
                if self.debug:
                    logger.debug('%s - %s' % (rtype, str(rule)))
    ...
    def overload_service(self, **args):
        ...
        for s in sources:
            ...
            self.add_rule(**rule)
            if self.debug:
                logger.debug('Overload %s ' % str(rule))

    def partition_services(self, **args):
        """
            Partitions two connected services. Not two sets of services (TODO)
            Expects usual arguments and srcprobability and dstprobability,
            that indicates probability of
